;;; day10 --- solving day 10 -*- lexical-binding: t; -*-
...
(defvar *q-bots*             nil)
(defvar *q-min-search-chip*  nil)
(defvar *q-max-search-chip*  nil)
(defvar *q-instructions*     nil)
(defvar *q-result*           nil)
(defvar *q-outputs*          nil)
(defvar *q-multiply-outputs* nil)

(defun q-give-bot (bot value)
  "Give BOT chip with value VALUE.

Bot is created if it's not there.
Bot is run if it has two chips."
  (let ((found-bot (gethash bot *q-bots*)))
    (pcase found-bot
      (`()
       (puthash bot (list nil nil nil nil value nil) *q-bots*))
      (`(nil nil nil nil ,val1 nil)
       (puthash bot (list nil nil nil nil val1 value) *q-bots*))
      (`(,dest-low ,dest-low-type ,dest-high ,dest-high-type nil nil)
       (puthash bot (list dest-low dest-low-type dest-high dest-high-type value nil) *q-bots*))
      (`(,dest-low ,dest-low-type ,dest-high ,dest-high-type ,val1 nil)
       (q-give-and-update bot dest-low dest-high dest-low-type dest-high-type val1 value)))))

(defun q-give-and-update (bot dest-low dest-high dest-low-type dest-high-type val1 val2)
  "Perform the giving and update the bot which gave."
  (let ((max-chip (max val1 val2))
        (min-chip (min val1 val2)))
    (if (and (not *q-multiply-outputs*)
             (= min-chip *q-min-search-chip*)
             (= max-chip *q-max-search-chip*))
        (progn
          (setq *q-instructions* nil)
          (setq *q-result*       bot))
      (progn
        (if (eq dest-low-type 'BOT)
            (q-give-bot dest-low min-chip)
          (puthash dest-low min-chip *q-outputs*))
        (if (eq dest-high-type 'BOT)
            (q-give-bot dest-high max-chip)
          (puthash dest-high max-chip *q-outputs*)))
      (puthash bot (list dest-low dest-low-type dest-high dest-high-type nil nil) *q-bots*))))

(defun q-give-bot-instruction (bot dest-low dest-high dest-low-type dest-high-type)
  "Instruct bot labelled BOT to give low to DEST-LOW and high to DEST-HIGH.

If DEST-LOW-TYPE is OUT, then just remove the value, likewise for
DEST-HIGH-TYPE."
  (let ((found-bot (gethash bot *q-bots*)))
    (pcase found-bot
      (`()
       (puthash bot
                (list dest-low dest-low-type dest-high dest-high-type nil nil)
                *q-bots*))
      (`(nil nil nil nil ,val1 nil)
       (puthash bot (list dest-low dest-low-type dest-high dest-high-type val1 nil) *q-bots*))
      (`(nil nil nil nil ,val1 ,val2)
       (q-give-and-update bot dest-low dest-high dest-low-type dest-high-type val1 val2)))))

(defun q-run-instructions ()
  "Run the current `*q-instructions*' exit when a comparison is flagged."
  (progn
    (while *q-instructions*
      (let ((instruction (pop *q-instructions*)))
        (pcase instruction
          (`(GIVE ,value ,bot)
           (q-give-bot bot value))
          (`(COMP ,bot ,dest-low-type ,dest-low ,dest-high-type ,dest-high)
           (q-give-bot-instruction bot dest-low dest-high dest-low-type dest-high-type)))))
    (if *q-multiply-outputs*
        (* (gethash 0 *q-outputs*)
           (gethash 1 *q-outputs*)
           (gethash 2 *q-outputs*))
      *q-result*)))
...
Instructions are parsed into:
 - (GIVE     value bot)
 - (COMP bot   BOT|OUT dest-high BOT|OUT dest-low)
 - (COMP bot   BOT|OUT dest-high BOT|OUT dest-low)"
  (let ((split (split-string line " ")))
    (pcase split
      (`("value" ,value "goes" "to" "bot" ,bot)
       `(GIVE ,(string-to-number value) ,(string-to-number bot)))
      (`("bot" ,bot "gives" "low" "to" "bot" ,dest-low "and" "high" "to" "bot" ,dest-high)
       `(COMP ,(string-to-number bot) BOT ,(string-to-number dest-low) BOT ,(string-to-number dest-high)))
      (`("bot" ,bot "gives" "low" "to" "output" ,dest-low "and" "high" "to" "bot" ,dest-high)
       `(COMP ,(string-to-number bot) OUT ,(string-to-number dest-low) BOT ,(string-to-number dest-high)))
      (`("bot" ,bot "gives" "low" "to" "bot" ,dest-low "and" "high" "to" "output" ,dest-high)
       `(COMP ,(string-to-number bot) BOT ,(string-to-number dest-low) OUT ,(string-to-number dest-high)))
      (`("bot" ,bot "gives" "low" "to" "output" ,dest-low "and" "high" "to" "output" ,dest-high)
       `(COMP ,(string-to-number bot) OUT ,(string-to-number dest-low) OUT ,(string-to-number dest-high))))))

(defun q-parse-instructions (bytefile)
  "Parse a collection of *q-instructions* from lines in BYTEFILE."
  (mapcar #'q-parse-instruction (split-string bytefile "\n" t " ")))

(defun q-answer-question (input)
  "Parse *q-instructions* from INPUT and find the responsible bot."
  (interactive "sInput: ")
  (let ((*q-bots*             (make-hash-table))
        (*q-outputs*          (make-hash-table))
        (*q-min-search-chip*  17)
        (*q-max-search-chip*  61)
        (*q-instructions*     (q-parse-instructions input))
        (*q-multiply-outputs* (y-or-n-p "Multiply outputs? "))
        (*q-result*           nil))
    (message "%s" (q-run-instructions))))

;; Simple answer:       161
;; Multpilying outputs: 133163
