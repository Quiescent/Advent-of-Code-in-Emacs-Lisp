;;; day4 --- solving day 4 -*- lexical-binding: t; -*-
...
(defun q-parse (input)
  "Produce a list of room descriptions from INPUT.

The triples contain, in order, the room name, sector id and
checksum."
  (cl-labels ((parse-line (line)
                          (progn (string-match "\\([-a-z]*\\)-\\([0-9]+\\)*\\[\\([a-z]*\\)\\]" line)
                                 (list (substring line (match-beginning 1) (match-end 1))
                                       (string-to-number (substring line (match-beginning 2) (match-end 2)))
                                       (substring line (match-beginning 3) (match-end 3))))))
    (mapcar #'parse-line (split-string input "\n" t))))

(defun q-add-freq-data (room)
  "Produce ROOM with added character frequency data."
  (let ((frequency-data (make-char-table nil 0)))
    (seq-do (lambda (char)
              (when (not (= char ?-))
                (aset frequency-data char
                      (1+ (aref frequency-data char)))))
            (car room))
    (cons frequency-data room)))

(defun q-valid-room-p (room)
  "Produce t if ROOM is valid.
...
Does not produce a pair if it was associated with nil or 0."
  (let (result)
    (dolist (char '(?a ?b ?c ?d ?e ?f ?g ?h ?i ?j ?k ?l ?m ?n ?o ?p ?q ?r ?s ?t ?u ?v ?w ?x ?y ?z) result)
      (let ((freq (aref char-table char)))
        (when (and (not (null freq))
                   (> freq 0))
          (push (cons char freq) result))))))

(defun q-compute-checksum (freq-data)
  "Produce the checksum of FREQ-DATA."
  (let* ((alist    (q-char-table-pairs freq-data))
         (sorted   (sort alist (pcase-lambda (`(,_ . ,count-this) `(,_ . ,count-that))
                                 (> count-this count-that))))
         (curr-str "")
         (prev-cnt (cdar sorted))
         (curr-cnt nil)
         (result   ""))
    (dolist (key-value sorted result)
      (setq curr-cnt (cdr key-value))
      (when (/= curr-cnt prev-cnt)
        (setq result (concat result (seq-sort #'< curr-str)))
        (setq curr-str ""))
      (setq curr-str (concat curr-str (char-to-string (car key-value))))
      (setq prev-cnt curr-cnt))
    (substring (concat result (seq-sort #'< curr-str)) 0 5)))

(defun q-decrypt-name (room)
  "Decrypt the name of ROOM."
  (pcase room
    (`(,_freq-data ,name ,code ,_checksum)
     (mapconcat #'char-to-string
                (seq-map
                 (lambda (x)
                   (if (= x ?-)
                       ?\  (+ ?a (mod (+ (- x ?a) code) 26)))) name) ""))))

(defun q-find-room (rooms)
  "Find the room with the north pole ojbects in ROOMS."
  (cl-find-if (lambda (room)
                (string= q-north-pole-objects-room (q-decrypt-name room)))
              rooms))

(defun q-answer-question (input)
  "Sum the sector ids of the valid rooms described by INPUT."
  (interactive "sInput: ")
  (let* ((parsed         (q-parse input))
         (with-freq-data (mapcar #'q-add-freq-data parsed))
         (valid-rooms    (cl-remove-if-not #'q-valid-room-p with-freq-data)))
    (if (y-or-n-p "Sum room codes? ")
        (message "%s" (seq-reduce (pcase-lambda (acc `(,_freq-data ,_name ,code ,_checksum))
                                    (+ code acc))
                                  valid-rooms 0))
      (message "%s" (caddr (q-find-room valid-rooms))))))

;; simple answer:     278221
;; room with objects: 267
