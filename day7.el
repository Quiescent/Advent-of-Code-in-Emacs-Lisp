;;; day7 --- solving day 7 -*- lexical-binding: t; -*-
...
(defun q-parse-regexp (regexp s f tag)
  "Parse REGEXP from head of S consing it onto result of (F rest-of-string).

When s is nil, produce nil.  Parsed string is tagged with TAG."
  (if (string= s "")
      nil
    (progn
      (when (string-match regexp s)
        (let ((end   (match-end 1))
          ...
          (cons (cons tag (substring s start end))
                (funcall f (substring s (match-end 0)))))))))

(defun q-parse-hypernet (ip-string)
  "Parse a hypernet from IP-STRING and then parse an ordinary section."
  (q-parse-regexp "\\[\\([a-z]+\\)\\]" ip-string
                  #'q-parse-ordinary 'HYPERNET))

(defun q-parse-ordinary (ip-string)
  "Parse an ordinary section of the IP-STRING and then parse a hypernet."
  (q-parse-regexp "\\([a-z]+\\)" ip-string
                  #'q-parse-hypernet 'ORDINARY))

(defun q-parse-ip (ip-string)
  "Parse IP-STRING into a list of lists annotated as HYPERNET and ORDINARY."
  (let ((parsed       (q-parse-ordinary ip-string))
        (split-parsed (cons (list 'HYPERNET)
                            (list (list 'ORDINARY)))))
    (mapcar #'nreverse (dolist (section parsed split-parsed)
                         (if (eq (car section) 'HYPERNET)
                             (push (cdr section) (car split-parsed))
                           (push (cdr section) (cadr split-parsed)))))))

(defun q-contains-abba (s)
  "Produce the index at which S has a `*q-palindrome-length*' palindrome in it.

The first two letters in the palindrome must be different.
Produce nil if there isn't one."
  (let ((i        0)
        (half-len (/ *q-palindrome-length* 2))
        hit-positions)
    (cl-labels
        ((is-abba ()
                  (let ((l       0)
                        (matched '())
                        (offset  (1- *q-palindrome-length*))
                        test-char)
                    (while (let* ((start (+ i l)))
                             (and (= (setq test-char (aref s start))
                                     (aref s (+ start offset)))
                                  (not (member test-char matched))
                                  (< l half-len)))
                      (cl-incf l)
                      (push test-char matched)
                      (cl-decf offset 2))
                    (and (= l half-len)
                         (if (= 1 (mod *q-palindrome-length* 2))
                             (not (member (aref s (+ i l)) matched))
                           t)))))
      (while (<= (+ i *q-palindrome-length*) (length s))
        (when (is-abba)
          (push i hit-positions))
        (cl-incf i)))
    hit-positions))

(defun q-ip-contains-abba (ip)
  "Produce t if any ordinary sequence in IP has an abba."
  (and (cl-find-if #'q-contains-abba (cdadr ip))
       (not (cl-find-if #'q-contains-abba (cdar ip)))))

(defun q-ip-abas (ip)
  "Produce all of the abas in IP."
  (let* ((*q-palindrome-length* 3)
         (positions (mapcar #'q-contains-abba
                            (cdadr ip)))
         abas)
    (mapcar* (lambda (hits ip-part)
               (mapc (lambda (hit)
                       (push (substring ip-part
                                        hit
                                        (+ hit *q-palindrome-length*))
                             abas))
                     hits))
             positions
             (cdadr ip))
...
(defun q-ip-supports-ssl (ip)
  "Produce t if the IP has an ABA and a BAB."
  (let ((abas (q-ip-abas ip)))
    (when abas
      (cl-labels ((aba-to-bab (aba)
                              (let ((new-aba (substring aba 0))
                                    (a       (aref aba 0))
                                    (b       (aref aba 1)))
                                (aset new-aba 0 b)
                                (aset new-aba 2 b)
                                (aset new-aba 1 a)
                                new-aba)))
        (let ((babs (mapcar #'aba-to-bab abas)))
          (cl-find-if (lambda (bab)
                        (cl-find-if (lambda (x) (string-match bab x))
                                    (cdar ip)))
                      babs))))))

(defun q-answer-question (input)
  "Produce the count of the ips in INPUT which have an abba."
  (interactive "sInput: ")
  (if (y-or-n-p "Count ips with ssl? ")
      (message "%s" (cl-count-if #'q-ip-supports-ssl
                                 (mapcar #'q-parse-ip
                                         (split-string input "\n" t))))
    (let ((*q-palindrome-length* 4))
      (message "%s" (cl-count-if #'q-ip-contains-abba
                                 (mapcar #'q-parse-ip
                                         (split-string input "\n" t)))))))
