;;; day9 --- solving day 9 -*- lexical-binding: t; -*-

;;; Commentary:

;; Input (a very long line of text :P):
...
(defvar *q-input-stream*     nil)
(defvar *q-inner-input*      nil)
(defvar *q-output-stream*    nil)
(defvar *q-version-two-mode* nil)
(defvar *q-cache*            nil)

(defconst q-parse-marker-regexp "([0-9]+x[0-9]+)")

(defun q-push-output (start end)
  "Put the region [START, END) from the input stream and put it on the output."
  (setq *q-output-stream* (concat *q-output-stream*
                                  (substring *q-input-stream* start end))))

(defun q-pop-input (start end)
  "Remove the region [START, END) from the input stream."
  (setq *q-input-stream* (concat (substring *q-input-stream* 0 start)
                                 (substring *q-input-stream* end))))

(defun q-make-key (n length input-stream end)
  "Make a key for a hash table with N and a LENGTH substring from INPUT-STREAM starting at END."
  (concat (number-to-string n) "-" (substring input-stream end (+ end length))))

(defun q-memoized-parse (length n end)
  "Push, [0, LENGTH) N times and pop input up to END.

If this input, including the input stream sub state, has been
seen before then return that answer."
  (let* ((key          (q-make-key n length *q-input-stream* end))
         (prev         (gethash key *q-cache*))
         (output-start (length *q-output-stream*)))
    (progn
      (if prev
          (progn
            (setq *q-output-stream* (concat *q-output-stream* prev))
            (q-pop-input 0 (+ end length)))
        (progn
          (dotimes (_ n)
            (q-push-output end (+ end length)))
          (q-pop-input 0 (+ end length))
          (puthash key (substring *q-output-stream* output-start) *q-cache*)))
      (when (and *q-version-two-mode*
                 (string-match q-parse-marker-regexp *q-output-stream*))
        (setq *q-input-stream*  (concat *q-output-stream* *q-input-stream*))
        (setq *q-output-stream* "")
        (q-parse-marker)))))

(defun q-parse-marker ()
  "Parse a marker from the current input stream."
  (if (string-match q-parse-marker-regexp *q-input-stream*)
      (let* ((start  (match-beginning 0))
             (end    (match-end       0))
             (marker (mapcar #'string-to-number (split-string (substring *q-input-stream*
                                                                         (1+ start) (1- end)) "x" t))))
        (q-push-output 0 start)
        (pcase marker
          (`(,length ,times)
           (q-memoized-parse length times end))))
    (progn
      (let ((len (length *q-input-stream*)))
        (q-push-output 0 len)
        (q-pop-input   0 len)))))

(defun q-decompress (input)
  "Setup the input and output streams and parse INPUT."
  (let* ((*q-input-stream*    input)
         (*q-output-stream*   "")
         (*q-inner-input*     "")
         (*q-cache*           (make-hash-table :test #'equal))
         (loop-test         (if *q-version-two-mode*
                                (lambda () (string-match q-parse-marker-regexp *q-input-stream*))
                              (lambda() (not (string= "" *q-input-stream*))))))
    (while (funcall loop-test)
      (q-parse-marker))
    *q-output-stream*))

(defun q-b-decompress-count-iter (input)
  "Parse INPUT using a temporary buffer."
  (with-temp-buffer
    (let ((count (math-bignum 0)))
      (insert input)
      (goto-char (point-min))
      (while (and (re-search-forward q-parse-marker-regexp nil t)
                  (< (point) (point-max)))
        (kill-sexp -1)
        (let ((start (point))
              (sexp  (car kill-ring)))
          (pcase (mapcar #'string-to-number (split-string (substring sexp 1 (1- (length sexp))) "x" t))
            (`(,length ,times)
             (kill-region start (+ start length))
             (if *q-version-two-mode*
                 (let* ((input (car kill-ring)))
                   (if (string-match q-parse-marker-regexp input)
                       (setq count (math-add count
                                             (math-mul (math-bignum times)
                                                       (q-b-decompress-count-iter input))))
                     (setq count (math-add count
                                           (math-bignum (* length times))))))
               (setq count (math-add count (math-bignum (* length times)))))))))
      (math-add count (math-bignum (- (point-max) (point-min)))))))

(defun q-b-decompress-count (input)
  "Use buffers and intelligent caching to decompress INPUT."
  (let ((*q-cache* (make-hash-table :test #'equal)))
    (q-b-decompress-count-iter input)))

(defun q-answer-question (input)
  "Produce the length of INPUT, decompressed."
  (interactive "sInput: ")
  (let* ((*q-version-two-mode* (y-or-n-p "Use version 2? "))
         (decompressed-count (math-format-number (q-b-decompress-count input))))
    (message "%s" decompressed-count)))

;; Length of answer:        98135
;; Length in advanced mode: 10964557607
